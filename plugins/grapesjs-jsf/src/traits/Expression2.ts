...
import { Editor, Modal, Page } from 'grapesjs';
...
export const newPageDialog = (editor: Editor, opts: any) => {
...
function onImport(editor: Editor, page: Page) {
  editor.Pages.select(page)
  editor.runCommand('gjs-open-import-webpage')
}

function displayDialog(editor: Editor, config: any, page: Page) {
  render(html`
    <form class="silex-form">
      ...
      <footer>
        <input class="silex-button" type="button" @click=${(e:PointerEvent) => onImport(editor, page)} value="Import from website">
        <input class="silex-button" type="button" @click=${(e: PointerEvent) => editor.stopCommand(cmdOpenNewPageDialog)} value="Cancel">
...
function saveDialog(editor: Editor, config: any, page: Page) {
  const form = el.querySelector('form')
  if (form) {
